...
export function MembersGrid() {
  return (
    <motion.div
      ...
      className="grid gap-1 grid-cols-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8"
    >
      {members.map((member) => (
        <div
          key={member.id}
            ...
            <div className="text-center text-white p-4">
              ...
              <div className="mb-3">
                <div className="flex flex-wrap gap-1 justify-center">
                  {member.activities.slice(0, 1).map((activity, idx) => (
                    <span
                      key={idx}
                      className="px-2 py-1 bg-black bg-opacity-30 text-white text-xs border border-white border-opacity-50"
                    >
                      {activity}
                    </span>
                  ))}
                  {member.activities.length > 1 && (
                    <span className="px-2 py-1 bg-white bg-opacity-10 text-white text-xs">
                      +{member.activities.length - 1}
                    </span>
                  )}
                </div>
